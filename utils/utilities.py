...
def set_dpi_awareness() -> None:# Windows can apply per-monitor DPI scaling (100 %, 125 %, 150 %, …)
    """Attempt to mark the process as DPI‑aware on Windows.
    Called before creating the pygame window or initializing SDL so that the underlying window is created with the correct size
    """
...
def get_dpi_scale(hwnd: int | None = None) -> float:
    ...
    #On Windows the default DPI is 96, so a DPI report of 120 corresponds to a scale factor of 1.25 (125%).  If the function fails for any reason, it defaults to a scale factor of 1.0 (100%).
